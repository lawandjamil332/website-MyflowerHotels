...
const hotelPanel = (g: Gathered) => {
  ...
  return panel(
    ...
    `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
      ...
      ${branch.phone ? row(t.email.lPhone, `<a href="tel:${esc(branch.phone.replace(/\s/g, ''))}" style="color:#705129;text-decoration:none;" dir="ltr">${esc(branch.phone)}</a>`, dir) : ''}
    </table>`,
    dir,
  )
}
...
const guestPanel = (g: Gathered) => {
  ...
  return panel(
    ...
    `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
      ...
      ${row(t.email.lPhone, `<a href="tel:${esc(booking.guestPhone.replace(/\s/g, ''))}" style="color:#705129;text-decoration:none;" dir="ltr">${esc(booking.guestPhone)}</a>`, dir, { strong: true })}
      ${booking.guestEmail ? row(t.email.lEmail, `<a href="mailto:${esc(booking.guestEmail)}" style="color:#705129;text-decoration:none;" dir="ltr">${esc(booking.guestEmail)}</a>`, dir) : ''}
