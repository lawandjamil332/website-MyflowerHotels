...
/**
 * What is left to sell, room by room, night by night.
 *
 * This is the screen the extranet is actually built around. Booking.com calls
 * it Rates & Availability and it is a grid: your room types down the side,
 * the next fortnight across the top, and in every cell the number of that room
 * still free that night. A hotel manager looks at it and knows what to do —
 * which is not a question the panel could answer at all before, because the
 * only way to see whether the 14th was full was to open the booking form and
 * try to book it.
 *
 * The counting is done here rather than in SQL. One query fetches the stays
 * that touch the fortnight — at four hotels that is tens of rows, not
 * thousands — and the overlap arithmetic happens in JavaScript, where it can
 * be read. A date-series join would be cleverer and nobody would ever check it.
 *
...
export type Availability = {
  /** Midnight UTC of the first column. */
  from: string
  dates: string[]
  rooms: RoomRow[]
  /** Rooms sold across the whole group, per night, and the group's stock. */
...
/**
 * The first column. A date in the query string wins, so the arrows either side
 * of the grid are ordinary links and the view is shareable; anything
 * unreadable falls back to today rather than to an error.
 */
export const windowStart = (from?: string): Date => {
  if (from) {
...
export const runAvailability = async (from?: string): Promise<Availability | null> => {
  try {
    ...
    const [rooms, stays] = await Promise.all([
      ...
      // Every stay that overlaps the fortnight at all: starts before it ends,
      // ends after it starts. Touching stays do not overlap — somebody
      // checking out on the 5th frees the room for somebody arriving on the 5th.
    ...
    // room id -> how many rooms are taken on each of the fortnight's nights.
